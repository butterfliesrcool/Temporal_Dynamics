...
bat_HWB$real_1 <- bat_HWB$percent_reflectance
library(dplyr)
# Remove columns using select()
bat_HWB <- bat_HWB %>% select(-c(percent_reflectance))
...
bat_HWB_plot <- aggplot(bat_HWB, type, 
        ...
        alpha = 0.3, 
        lty = 2:1, 
        lcol = c("darkred", "darkblue"), 
        shadecol = c("darkred", "darkblue"),
        lwd = 2, 
        legend = FALSE)

legend(440, 9, legend=c("Real Wings", "Facsimile"),
       col=c("darkblue", "darkred"), lty=1:2, lwd = 2, box.lty=0)
text(600, 50, "JND = 1.43")
```
...
vis.model
JND <- coldist(vis.model,noise = 'neural', n = c(0.37, 0.71, 0.99, 1.0)) 
JND
...
legend(300, 50, legend=c("Real Wing", "Facsimile"),
       col=c("darkblue", "darkred"), lty=1:2, lwd = 2, box.lty=0)
text(650, 46, "JND = 1.43")

```
```{r}
library(patchwork)

figure = bat_HWB_plot + lim_HWB_plot
figure
```
...
vis.model2
JND2 <- coldist(vis.model2,noise = 'neural', n = c(0.37, 0.71, 0.99, 1.0)) 
JND2
# Warning: Quantum catch are relative, distances may not be meaningful 
```
